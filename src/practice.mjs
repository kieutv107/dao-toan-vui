...
import {createPractice,current,answer,markHint,pickGame} from './practice-engine.mjs';
import {strategyHint} from './strategies.mjs';
import {showCenterCheck,showMiss,announce,NEXT_DELAY_MS} from './feedback.mjs';

export function mountPractice(app,{home,beep,learning,startGame,games,focusLevel}){
  const before=learning.summary(),topic=focusLevel?before.levels.find(l=>l.id===focusLevel):null,g=createPractice({profile:learning.profile,sessionId:learning.newSessionId(),record:e=>learning.record(e),focusLevel:topic?.id}),$=s=>app.querySelector(s);
  let options=[],bad=[],locked=false,disposed=false,advanceTimer=null,message='',kind='',startedAt=performance.now();
  app.innerHTML=`<div class="play-top"><button class="back" id="practice-back">← Đảo trò chơi</button><span>🌱 Vườn luyện tập</span><span class="practice-badge">${topic?`Chặng ${topic.id} · ${topic.title}`:'Một lượt nhỏ · 18 câu chính'}</span></div><section class="play green challenge"><div class="challenge-hud" id="practice-hud"></div><p class="challenge-rules">${topic?`Lượt này chỉ luyện chặng “${topic.title}” bé đã chọn. Kết quả vẫn ghi vào hành trình của bé; chương trình vẫn mở chặng dần dần.`:'Game sẽ chọn đúng phần bé đang cần.'} Câu chưa vững sẽ được ôn lại sau vài câu. Không tính giờ, không trừ điểm!</p><div id="practice-body"></div></section>`;
  ...
  function choose(value,button){if(locked||disposed||g.done||bad.includes(value))return;const q=current(g),result=answer(g,value,{elapsedMs:performance.now()-startedAt});
    ...
    locked=true;button.classList.add('right','challenge-pop');app.querySelectorAll('[data-answer]').forEach(b=>b.disabled=true);$('#hint').disabled=true;beep();showCenterCheck($('.play'));feedback('','success');announce($('#feedback'),`Chính xác, ${q.a} ${q.sign} ${q.b} = ${q.answer}`);hud();advanceTimer=setTimeout(()=>result.complete?finish():newQuestion(true),NEXT_DELAY_MS);
  }
  function finish(){if(disposed)return;const after=learning.summary(),gained=Math.max(0,after.mastered-before.mastered),suggestion=pickGame(games);
    $('#practice-body').innerHTML=`<div class="finish-icon">${gained?'🏆':'🌟'}</div><h2>${gained?`Bé vừa thuộc thêm ${gained} phép tính!`:'Hoàn thành một lượt luyện!'}</h2><p>Bé đã làm đúng ${g.results.correct} câu. Hiện có ${after.learning} phép đang luyện và ${after.mastered} phép đã thuộc.</p><div class="finish-actions"><button class="primary" id="practice-again">${topic?'↻ Luyện thêm chặng này':'↻ Luyện thêm'}</button><button class="primary secondary" id="practice-recommend">${suggestion.icon} Chơi ${suggestion.title}</button><button class="back" id="practice-home">Nghỉ một chút</button></div>`;
    $('#practice-home').onclick=home;$('#practice-recommend').onclick=()=>startGame(suggestion.id);$('#practice-again').onclick=()=>startGame('practice',topic?{focusLevel:topic.id}:{});$('#practice-again').focus();hud();
  }
