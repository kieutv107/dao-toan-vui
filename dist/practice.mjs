...
export function mountPractice(app,{home,beep,learning,startGame}){
  ...
  app.innerHTML=`<div class="play-top"><button class="back" id="practice-back">← Đảo trò chơi</button><span>🌱 Vườn luyện tập</span><span class="practice-badge">Một lượt nhỏ · 18 câu</span></div><section class="play green challenge"><div class="challenge-hud" id="practice-hud"></div><p class="challenge-rules">Game sẽ chọn đúng phần bé đang cần. Không tính giờ, không trừ điểm — cứ bình tĩnh thử nhé!</p><div id="practice-body"></div></section>`;
  $('#practice-back').onclick=home;
  function hud(){$('#practice-hud').innerHTML=`<div><span>Câu</span><b>${Math.min(g.pos+1,g.questions.length)}/${g.questions.length}</b></div><div><span>Đã làm đúng</span><b>${g.results.correct}</b></div><div><span>Đang luyện</span><b>${learning.summary().learning}</b></div><div><span>Đã thuộc</span><b>${learning.summary().mastered}</b></div>`}
  ...
  function render(q){
    $('#practice-body').innerHTML=`<div class="play-label">CÂU ${Math.min(g.pos+1,g.questions.length)} / ${g.questions.length}</div><h2>Mình cùng tính nhé!</h2><div class="equation">${q.a}<span>${q.sign}</span>${q.b}<span>=</span><b class="unknown">?</b></div><div class="answers">${options.map((v,i)=>`<button data-answer="${v}" style="--i:${i}">${v}</button>`).join('')}</div><div id="feedback" role="status" aria-live="polite" class="challenge-feedback"></div><button class="hint" id="hint">💡 Cho bé một gợi ý</button><div id="hint-content"></div>`;
