...
test('new profile starts in 11–15 and keeps every practice question in that band',()=>{
  const p=createProfile();assert.equal(unlockedBand(p),2);
...
test('less than seventy percent strong facts keeps the child in 11–15',()=>{
  ...
  assert.equal(unlockedBand(p),2);
});

test('seventy percent strong facts unlocks 16–20',()=>{
...
test('hardest selection prefers weak previously missed fact',()=>{
  const p=createProfile(),candidates=factCatalog().filter(f=>f.band===2),weak=candidates[4],strong=candidates[5];
